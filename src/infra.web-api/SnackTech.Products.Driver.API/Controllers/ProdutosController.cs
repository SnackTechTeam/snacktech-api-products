...
public class ProdutosController(ILogger<ProdutosController> logger, IProdutoController produtoController)
    : CustomBaseController(logger)
{
    /// <summary>
    ...
    {
        return await ExecucaoPadrao("Produtos.Post", produtoController.CadastrarNovoProduto(novoProduto));
    }
    ...
    {
        return await ExecucaoPadrao("Produtos.Put", produtoController.EditarProduto(identificacao, produtoEditado));
    }
    ...
    {
        return await ExecucaoPadrao("Produtos.Delete", produtoController.RemoverProduto(identificacao));
    }
    ...
    [SwaggerOperation(Summary =
        "Retorna todos os produtos de uma categoria informada. Categorias cadastradas: 1. Lanche, 2. Acompanhamento, 3. Bebida, 4. Sobremesa")]
    public async Task<IActionResult> GetByCategory(int categoriaId)
    {
        return await ExecucaoPadrao("Produtos.GetPorCategoria",
            produtoController.BuscarProdutosPorCategoria(categoriaId));
    }
    ...
    {
        return await ExecucaoPadrao("Produtos.GetPorId", produtoController.BuscarProdutoPorId(identificacao));
    }
}